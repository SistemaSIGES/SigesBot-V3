...
const flujoAplicaciones = addKeyword("__flujoAplicaciones__", { sensitive: true })
  .addAnswer(
    "Elija en qué aplicación está teniendo el inconveniente\n1. App Propia (YVOS-PRIS-ON-BOX-ETC)\n2. Mercado Pago - Modo - ETC\n3. Todas las aplicaciones",
    ...
    async (ctx, { fallBack, provider, state, gotoFlow }) => {
      ...
      switch (ctx.body) {
        case "1":
          await state.update({ typeProblem: "App Propia (YVOS-PRIS-ON-BOX-ETC)" });
          break;
        case "2":
          await state.update({ typeProblem: "Mercado Pago - Modo - ETC" });
          break;
        case "3":
          await state.update({ typeProblem: "Todas las aplicaciones" });
          break;
        default:
          await respuesta(
            ctx.from,
...
export default flujoAplicaciones;
