...
const flujoImpresoraComun = addKeyword("__flujoImpresoraComun__", { sensitive: true })
  .addAnswer(
    "Elija la opcion deseada\n1. Soporte para impresora\n2. Instalar una impresora",
    ...
    async (ctx, { fallBack, provider, state, gotoFlow }) => {
      ...
      switch (ctx.body) {
        case "1":
          await state.update({ typeProblem: "Soporte para impresora" });
          break;
        case "2":
          await state.update({ typeProblem: "Instalar una impresora" });
          break;
        default:
...
export default flujoImpresoraComun;
