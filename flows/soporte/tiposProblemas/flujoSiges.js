...
const flujoSiges = addKeyword("__flujoSiges__", { sensitive: true })
  .addAnswer(
    "Elija el origen del problema\n1. Facturación\n2. Cierre de turno\n3. Informes\n4. Otro",
    ...
    async (ctx, { fallBack, provider, state, gotoFlow }) => {
      ...
      switch (ctx.body) {
        case "1":
          await state.update({ typeProblem: "Facturación" });
          break;
        case "2":
          await state.update({ typeProblem: "Cierre de turno" });
          break;
        case "3":
          await state.update({ typeProblem: "Informes" });
          await respuesta(
            ctx.from,
            provider,
            "En la siguiente seccion indicar en que tipo de informe ocurre el problema"
          );
          break;
        case "4":
          await state.update({ typeProblem: "Otro" });
          break;
        default:
