...
const flujoImpresoraFiscal = addKeyword("__flujoImpresoraFiscal__", { sensitive: true })
  .addAnswer(
    "Elija la opcion deseada\n1. Soporte para impresora fiscal\n2. Instalar una impresora fiscal",
    ...
    async (ctx, { fallBack, provider, state, gotoFlow }) => {
      ...
      switch (ctx.body) {
        case "1":
          await state.update({ typeProblem: "Soporte para impresora fiscal" });
          break;
        case "2":
          await state.update({ typeProblem: "Instalar una impresora fiscal" });
          break;
        default:
...
export default flujoImpresoraFiscal;
